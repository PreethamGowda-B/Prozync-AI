...
export class LocalSandboxClient {
  ...
  async start(): Promise<void> {
    console.log(chalk.blue("🚀 Starting Prozync AI local sandbox..."));
    console.log(
      chalk.yellow(
  ...
  private async connect(): Promise<void> {
    console.log(chalk.blue("Connecting to Prozync AI backend..."));
...
export function main(): void {
  ...
  if (hasFlag("--help") || hasFlag("-h")) {
    console.log(`
${chalk.bold("Prozync AI Local Sandbox Client")}
...
`);
  ...
  if (!config.token) {
    ...
    console.error(
      chalk.yellow("Get your token from Prozync AI Settings > Agents"),
    );
    process.exit(1);
  }
